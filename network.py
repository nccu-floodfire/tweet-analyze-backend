...
def network(data, data2):
    ...
    df_embeddings = pd.DataFrame(user_embeddings_2d, columns=['from_user_name', 'x', 'y'])
    #製作立場分類需要的資料集
    ...
    encodings = tokenizer(new_texts, truncation=True, padding=True, max_length=512, return_tensors="pt")
    ...
    grouped = filtered_dataset.groupby('from_user_name')['prediction'].value_counts().unstack(fill_value=0)
    ...
    usertype = usertype[(usertype['類型'] == '新聞媒體') | (usertype['類型'] == '名人或意見領袖')]
    select = ['帳號 @user','類型']
    ...
    return output_degree, output_betweeness, output_closeness, output_eigenvector, result, filtered_dataset
