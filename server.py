...
#         return jsonify({'result': '上傳成功1'})
    ...
    if startDate1 and endDate1:
        ...
        data.to_csv(os.path.join(full_path, f'事件一：{startDate1}_{endDate1}.csv'), index=False)
    if startDate2 and endDate2:
        ...
        data.to_csv(os.path.join(full_path, f'事件二：{startDate2}_{endDate2}.csv'), index=False)
    if startDate3 and endDate3:
        ...
        data.to_csv(os.path.join(full_path, f'事件三：{startDate3}_{endDate3}.csv'), index=False)
...
def centerity(): #儲存社群網路資料及btm資料
    ...
    for file in files:
        if file.startswith('2'):
            ...
            betweeness_path = os.path.join(save_path5, f'{formatted_date}_betweeness.json')
            closeness_path = os.path.join(save_path5, f'{formatted_date}_closeness.json')
            eigenvector_path = os.path.join(save_path5, f'{formatted_date}_eigenvector.json')

            if not os.path.exists(path):
                combined_dataset = pd.read_csv(os.path.join(full_path, file))
                score, network_degree, network_betweeness, network_closeness, network_eigenvector, result, filtered_dataset = centerityScore(combined_dataset, path)
                ...
                if not os.path.exists(path1):
                    if dic_file:
                        topics_coords, terms_probs, top_5_doc = btm_analysis(combined_dataset, dic_path)
                    else:
                    ...
                    for topic, df in terms_probs.items():
                        path2 = os.path.join(save_path3, f'{formatted_date}_{topic}.csv')
                        df.to_csv(path2, index=False)
                    for topic, df in top_5_doc.items():
                        path3 = os.path.join(save_path4, f'{formatted_date}_{topic}.csv')
                ...
                if raw_predict_data.empty:
                    ...
                    raw_predict_data = pd.merge(raw_predict_data, filtered_dataset, on=['id'], how='outer', suffixes=('_raw', '_filtered'))
                    if 'prediction_filtered' in raw_predict_data.columns:
                        raw_predict_data.drop(columns='prediction_filtered', inplace=True)
                        raw_predict_data.rename(columns={'prediction_raw': 'prediction'}, inplace=True)
    ...
    if startDate1 and endDate1:
        ...
        data = pd.read_csv(os.path.join(full_path, f'事件一：{startDate1}_{endDate1}.csv'))
        path = os.path.join(save_path, f'事件一：{startDate1}_{endDate1}.csv')
        degree_path = os.path.join(save_path5, f'事件一：{startDate1}_{endDate1}_degree.json')
        betweeness_path = os.path.join(save_path5, f'事件一：{startDate1}_{endDate1}_betweeness.json')
        closeness_path = os.path.join(save_path5, f'事件一：{startDate1}_{endDate1}_closeness.json')
        eigenvector_path = os.path.join(save_path5, f'事件一：{startDate1}_{endDate1}_eigenvector.json')

        event_scores1, event1_network_degree, event1_network_betweeness, event1_network_closeness, event1_network_eigenvector, result, filtered_dataset = centerityScore(data,path)
        ...
        if not os.path.exists(path1):
            ...
            topics_coords.to_csv(path1, index=False)
            for topic, df in terms_probs.items():
                path2 = os.path.join(save_path3, f'事件一：{startDate1}_{endDate1}_{topic}.csv')
                df.to_csv(path2, index=False)
            for topic, df in top_5_doc.items():
                path3 = os.path.join(save_path4, f'事件一：{startDate1}_{endDate1}_{topic}.csv')
                df.to_csv(path3, index=False)
        #下載立場原始資料
        if raw_predict_data.empty:
            raw_predict_data = filtered_dataset
    ...
    if startDate2 and endDate2: 
        ...
        endDate2 = datetime.strptime(endDate2, '%Y-%m-%d')
        endDate2 = endDate2.strftime('%Y%m%d')
        data = pd.read_csv(os.path.join(full_path, f'事件二：{startDate2}_{endDate2}.csv'))

        path = os.path.join(save_path, f'事件二：{startDate2}_{endDate2}.csv')
        degree_path = os.path.join(save_path5, f'事件二：{startDate2}_{endDate2}_degree.json')
        betweeness_path = os.path.join(save_path5, f'事件二：{startDate2}_{endDate2}_betweeness.json')
        closeness_path = os.path.join(save_path5, f'事件二：{startDate2}_{endDate2}_closeness.json')
        eigenvector_path = os.path.join(save_path5, f'事件二：{startDate2}_{endDate2}_eigenvector.json')
       
        event_scores2, event2_network_degree, event2_network_betweeness, event2_network_closeness, event2_network_eigenvector, result, filtered_dataset = centerityScore(data,path)
        ...
        if not os.path.exists(path1):
            ...
            topics_coords.to_csv(path1, index=False)
            for topic, df in terms_probs.items():
                path2 = os.path.join(save_path3, f'事件二：{startDate1}_{endDate1}_{topic}.csv')
                df.to_csv(path2, index=False)
            for topic, df in top_5_doc.items():
                path3 = os.path.join(save_path4, f'事件二：{startDate1}_{endDate1}_{topic}.csv')
                df.to_csv(path3, index=False)
        #下載立場原始資料
        if raw_predict_data.empty:
            raw_predict_data = filtered_dataset
    ...
    if startDate3 and endDate3:
        ...
        endDate3 = datetime.strptime(endDate3, '%Y-%m-%d')
        endDate3 = endDate3.strftime('%Y%m%d')
        data = pd.read_csv(os.path.join(full_path, f'事件三：{startDate3}_{endDate3}.csv'))

        path = os.path.join(save_path, f'事件三：{startDate3}_{endDate3}..csv')
        degree_path = os.path.join(save_path5, f'事件三：{startDate3}_{endDate3}_degree.json')
        betweeness_path = os.path.join(save_path5, f'事件三：{startDate3}_{endDate3}_betweeness.json')
        closeness_path = os.path.join(save_path5, f'事件三：{startDate3}_{endDate3}_closeness.json')
        eigenvector_path = os.path.join(save_path5, f'事件三：{startDate3}_{endDate3}_eigenvector.json')
         
        event_scores3, event3_network_degree, event3_network_betweeness, event3_network_closeness, event3_network_eigenvector, result, filtered_dataset = centerityScore(data,path)
        ...
        if not os.path.exists(path1):
            ...
            topics_coords.to_csv(path1, index=False)
            for topic, df in terms_probs.items():
                path2 = os.path.join(save_path3, f'事件三：{startDate1}_{endDate1}_{topic}.csv')
                df.to_csv(path2, index=False)
            for topic, df in top_5_doc.items():
                path3 = os.path.join(save_path4, f'事件三：{startDate1}_{endDate1}_{topic}.csv')
                df.to_csv(path3, index=False)
        #下載立場原始資料
        if raw_predict_data.empty:
            raw_predict_data = filtered_dataset
...
def btm_terms():
    ...
    for csv_file in files:
        if csv_file.endswith(f'{topic}.csv'):
            # print(csv_file)
            data = pd.read_csv(os.path.join(full_path, csv_file))  # 讀取 CSV 文件
            btm_terms[csv_file] = data.to_dict()  # 將 DataFrame 轉換為字典並存儲在結果中
...
def btm_doc():
    ...
    for csv_file in files:
        if csv_file.endswith(f'{topic}.csv'):
            data = pd.read_csv(os.path.join(full_path, csv_file))  # 讀取 CSV 文件
            top_5_doc[csv_file] = data.to_dict()  # 將 DataFrame 轉換為字典並存儲在結果中
    # btm_terms = data.to_json()
...
def stance():
    ...
    if col_index == 1:
        return jsonify({'result': 'Selected column is the first column, no previous column available.'})
    previous_col = compare.columns[col_index - 1]
    filtered_compare = compare[(compare[selected_col] != '無資料') & (compare[previous_col] != '無資料')]
    filtered_compare = filtered_compare[(filtered_compare[selected_col] != '無貼文判斷立場') & (filtered_compare[previous_col] != '無貼文判斷立場')]
    changes = filtered_compare[filtered_compare[selected_col] != filtered_compare[previous_col]]
    changes = changes.merge(data[['user']], left_index=True, right_index=True)
    results = changes.apply(lambda row: [row['user'], row[previous_col], row[selected_col]], axis=1).tolist()
...
#     print(full_path)
    ...
    olddata['created_at'] = olddata['created_at'].apply(lambda x: x.strftime("%Y-%m-%d %H:%M"))
    newdata['created_at'] = newdata['created_at'].apply(lambda x: x.strftime("%Y-%m-%d %H:%M"))
...
def timeline():
    ...
    first = pd.to_datetime(min(p['startDate'] for p in highlightPeriods)) - timedelta(days=1)
    first = first.strftime('%Y%m%d')
    last = pd.to_datetime(max(p['endDate'] for p in highlightPeriods)) + timedelta(days=1)
    last = last.strftime('%Y%m%d')
    print(highlightPeriods2)
    return jsonify({'highlightPeriods':highlightPeriods,'highlightPeriods2':highlightPeriods2, 'first':first, 'last':last})
